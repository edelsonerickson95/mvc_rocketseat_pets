...
class PersonFinderController(PersonFinderControllerInterface):
    def __init__(self, people_repository: PeopeRepositoryInterface) -> None:
        self.__people_repository = people_repository

    def find(self, person_id: int) -> dict:
        person = self.__finder_person_id_db(person_id)
        response = self.__format_response(person)
        return response

    def __finder_person_id_db(self, person_id: int) -> PeopleTable:
